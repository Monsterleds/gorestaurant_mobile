...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      const { data: foodDetails } = await api.get<Food>(
        `foods/${routeParams.id}`,
      );

      const allExtras = foodDetails.extras.map(extra => {
        extra.quantity = 0;

        return extra;
      });

      Object.assign(foodDetails, {
  ...
  const toggleFavorite = useCallback(() => {
    ...
    // Does not have a route to select a favorite food
    // await api.post('/favorite', )
  }, [isFavorite, food]);

  const cartTotal = useMemo(() => {
    let { value } = extras.reduce(
      (previousExtra, currentExtra) => {
        previousExtra.value += currentExtra.value * currentExtra.quantity;
  ...
  async function handleFinishOrder(): Promise<void> {
    // Does not have a route to add an order.
